function dataset = ...
  NLP_ReadFileReutersWithPOS(filename, n, vocabulary, posTags)
...
tok_rare = '{rare}';
[tag_rare, found] = NLP_FindWord(tok_rare, vocabulary);
if ~found
  dim_w = dim_w + 1;
  tag_rare = dim_w;
  vocabulary{tag_rare} = tok_rare;
end
...
while 1
  ...
  if (strncmp(line, 'Reuters-training/00', 19) || ...
      strncmp(line, 'Reuters-test/00', 15))

    % Get the document name
    n_documents = n_documents + 1;
    [dummy, line] = strtok(line, '/');
    document_id = sscanf(strtok(line, '/'), '%d');
  ...
  if ((use_line) && (pos > 0))
    ...
    dataset.documents(n_nGrams + [1:m]) = document_id;
